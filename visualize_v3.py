"""
可视化（BLIP-2 式 v3, 2026-08-28）: 原图 vs 重建（单次前向, 无采样步）。
重建目标 = 原始像素 patch (B,N,588) → 反归一化 0-255 → 直接显示。
...
def parse_args():
    ...
    p.add_argument("--out", default="output/phase1_v3_blip2/recon_visual.png")
    return p.parse_args()
...
def main():
    ...
    model.eval().cuda()
    print(f"[model] N={num_patches}, K={model.num_queries}, "
    ...
    plt.suptitle(f"BLIP-2 式重建 (K={model.num_queries} 压缩 token, "
                 f"DINO {'冻结' if model.freeze_dino else '解冻'})",
