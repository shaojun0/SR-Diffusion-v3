...
### 5.6 可视化验证（还原效果 vs 质心 baseline, 2026-08-27）

针对用户疑问「还原效果不错，看是不是又是常数」，用 `visualize_constant_check.py` 做了专门可视化:
**原图 | 真实 DINO 特征 | 模型 F_hat | 质心 baseline** 四列同 PCA 变换、同色标，并做图内/图间变异性分解。

**选图**: 从 256 张 test 中挑 patch 特征**空间变异性最大**的 4 张（std 8.1e-5 ~ 9.4e-5）——
若连这些图模型都输出常数，则常数结论铁证；输出: `recon_constant_check.png`。

**常数判定（64 张 test, fp32）**:

| 维度 | 真实 P | 模型 F | 判读 |
|---|---|---|---|
| 图内（跨位置）std | 5.67e-5 | **4.06e-6**（小 14 倍） | 模型图内≈常数 |
| 图间（跨图）std | 0.9871 | **0.9871（完全一致, 比值 1.000）** | 模型精确还原每图质心 |
| 真实特征能量 | 图内 0.1 (0.00%) / 图间 63857 (100.00%) | — | **目标本身无空间结构** |
| PCA 前 3 主成分解释率 | 0.0972 | — | 空间变异极弱 |

**像素统计佐证**（可视化图四列 unique colors）: 原图 190543 → 真实特征 25027 →
模型 F_hat **602** → 质心 261。

**结论**:
- 模型 F_hat 图内 std≈4e-6（≈常数）**不是退化** —— 真实目标图内能量仅占 0.00%，
  工地图片的 DINO patch 特征**没有可还原的空间结构**;
- 模型图间 std=0.9871 **精确等于真实** —— 它把「输出每图质心」学到了理论下限
  （全量 fp32 L1=3.84e-5 ≈ 质心 baseline 4e-5）;
- 「还原效果不错」= 模型收敛到目标的最优常数解，**依然是常数，但这是目标特性使然**，
  换成有空间结构的目标（像素/SVD/其他数据集）才能检验真正的空间还原能力。
...
| `2e86492` | visualize_constant_check: 还原 vs 质心可视化 + 图内/图间变异性分解（常数判定） |
