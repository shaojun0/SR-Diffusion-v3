...
- 最新机制分析（2026-09-07，后步归零的最短因果链 / 破缺对称性，见 §4）: `doc/2026-09-07/ANALYSIS_k3_why_later_steps_zero.md`、`doc/2026-09-07/DESIGN_v2_region_loss.md`（P1 设计，实现已还原见 §4.1）
...
## 4. 阶段性目标 / 未决问题（2026-09-07 更新）

### 4.1 待决：让"后步真干活"需破缺双对称（方案未定，留档待后续设计）
...
（条件中位数）⇒ 后步输出≈0 ⇒ 后区 register 收不到有效读出梯度 ⇒ 键塌缩成 cos≈0.97 冗余簇 ⇒
残差更不可预测——**"零增量自锁"**。这是自洽均衡，不是某一层单独的病（梯度确实会传回可训的
DINO，但读出上限 ~19 使"键分化"的收益≈0，键收到的主要是草稿纸式的趋同梯度，而非"携带可读
差异内容"的压力）。

**本质 = 双对称简并解；破缺对称性是把后步激活的必要条件**：

| 对称性 | 表现 | 破缺手段（候选） |
|---|---|---|
| **步间任务对称** | 每步监督在函数形式上等价（都做 `L1(累加_t, 整图)`、平权、各收 1 份梯度）——"哪一步干活"对损失中性 | **P1 分区域掩码损失**：step t 只监督自己对应的 patch 行区，制造 step-1 抢不走的**私有目标**；损失本身产生 register 分工压力 |
| **键间内容对称** | 后区键互为镜像（slice27_v2 位置 16..63 两两 cos 0.963–0.974，采样种子 cos 0.998+）——交换任意键系统不变 | **P2 F1/E2' 编码侧注入**：special 输入拼 `Linear(patch_feat)`，让键有"各自携带逐 patch 内容"的可能（可叠正交/负余弦/使用率正则） |

- 要点：① 必须**双破缺同时成立**——只破任务对称则键空无可读，只破内容对称则损失仍奖励后步归零；② 不是外部扰动/正则"推一把"，而是改损失使对称点**本身失稳**，让破缺方向长出梯度（§2.3 的循环反向转动）；③ 已证伪的单侧手段：去解耦/后步加权（17.46 vs 18.77）、课程 fade-in（只推迟同一均衡）、残差目标显式化（单独无效——零仍是残差不可预测时的最优）。
- 读出上限提醒：F3/F4 固定模板读出上限 ~19（S25/S64 单步独占梯度也只有 19.15/19.58）；破键对称后若需超 ~19，还要逐 patch 内容句柄（E2 类），另议。
- **验证判据（= 对称破缺的观测特征）**：① 键 cos 明显跌破 0.9、z_s within-std 从 ~0.14 回升；② 后步 \|W·Y_t\| ≫ 0.015；③ 累加曲线出真阶梯（覆盖区单调降）且 eval_recon 不劣化（历史对比一律用 `eval_recon`，勿用 eval_loss——口径已随 region_loss 变化）；④ 各步**隔离** L1 在自己区域上相近（不再 39 vs 61）。
