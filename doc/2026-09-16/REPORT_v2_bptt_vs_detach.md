...
- `doc/2026-09-07/ANALYSIS_k3_why_later_steps_zero.md` 记录的"后步输出≈0 / 零增量自锁"**被打破**：逐步曲线不再平。
...
**建议后续**
- 多 seed 复核 −30.8% 的幅度。
- 既然 BPTT 免费且后步已开始贡献，可**重测 README §4.1 的 P1 分区掩码损失**（`doc/2026-09-07/DESIGN_v2_region_loss.md`）：当时"只给私有目标无效"的结论是在 detach（后步无梯度）前提下得到的，**前提已经变了**。
