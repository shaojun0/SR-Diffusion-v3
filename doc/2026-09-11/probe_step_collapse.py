...
2026-09-11: 结构超参改从 model_info.json 读取（支持 self.stack 加宽: stack_dim /
decoder_depth / heads / mlp_ratio / decoder_dropout），以便对 stack 2x 模型与基线
用**同一份探针**跑出可比数字。旧产物（无这些字段）走原默认 8/4.0/2/0/0，与历史一致。
...
def main():
    ...
    # ── model_info.json: query_mask_mode + 全部结构超参 ──
    ...
    def _pick(name, default):
        """model_info.json 优先; 无字段/None 则用历史默认。"""
        if info is not None and info.get(name) is not None:
            return info[name]
        return default

    heads = int(_pick("heads", 8))
    mlp_ratio = float(_pick("mlp_ratio", 4.0))
    decoder_depth = int(_pick("decoder_depth", 2))
    stack_dim = int(_pick("stack_dim", 0))
    decoder_dropout = float(_pick("decoder_dropout", 0.0))
    K_eff = int(_pick("num_specials", K))
    print(f"[cfg] tag={args.tag} query_mask_mode={qmm} ckpt={args.ckpt}")
    print(f"[cfg] heads={heads} mlp_ratio={mlp_ratio} depth={decoder_depth} "
          f"stack_dim={stack_dim}(dim={1024}) dropout={decoder_dropout} K={K_eff}")
    ...
    model = SRPhase1V2(dinov2=dino, num_patches=N, dim=dino.config.hidden_size,
                       heads=heads, mlp_ratio=mlp_ratio, decoder_steps=STEPS,
                       decoder_depth=decoder_depth, skip_steps=0, max_steps=5,
                       num_specials=K_eff, query_mask_mode=qmm,
                       stack_dim=stack_dim, decoder_dropout=decoder_dropout)
    ...
    print(f"[model] K={model.num_specials} steps={model.decoder.steps} "
          f"mode={model.query_mask_mode} stack_dim={model.decoder.stack_dim} "
          f"depth={len(model.decoder.stack.layers)}")
    ...
    res = {
        ...
        "steps": STEPS, "num_specials": K_eff,
        "heads": heads, "decoder_depth": decoder_depth,
        "stack_dim": stack_dim, "decoder_dropout": decoder_dropout,
