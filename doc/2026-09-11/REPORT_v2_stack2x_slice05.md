...
**按用户决定，训练在 step≈2500 手动停止，不再重跑**（塌缩自 step 400 起已持续 2000+ 步，无恢复迹象）。
本次改动本身**不污染历史配置**：`stack_dim=0` 默认路径与 HEAD `6915f6b` 的 state_dict 键/值与
forward 输出**逐位相同**（见 §3），旧 checkpoint 双向可载。
