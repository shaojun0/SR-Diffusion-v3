...
OUT_ROOT = os.environ.get("COT_OUT", "/root/autodl-tmp/cot_full")
CANVAS = (1200, 900)          # 12:9
N = int(os.environ.get("COT_N", "0"))   # 0 = 全量
SEED = 20260918
...
def main():
    ...
    ap.add_argument("--datasets", default="", help="逗号分隔前缀；空=全部")
    ap.add_argument("--limit", type=int, default=0, help="每数据集上限；0=全量")
    ar = ap.parse_args()
    global N
    N = ar.limit if ar.limit else 10 ** 9   # 0 = 全量
    random.seed(SEED)
    os.makedirs(OUT_ROOT, exist_ok=True)
    manifest = []
    stats = {}
    skipped_nobox = [0]
    want_ds = [x for x in ar.datasets.split(",") if x]
    for name, cfg in DATASETS.items():
        if want_ds and not any(name.startswith(w) for w in want_ds):
            continue
        ...
        for f in files:
            ...
            for batch in pf.iter_batches(batch_size=16, columns=[cfg["img_col"]] + want):
                ...
                for r in rows:
                    ...
                    task = cfg["task"]
                    if name.startswith("aswin"):
                        task = "detect" if boxes else "caption"
                    elif cfg.get("require_boxes") and not boxes:
                        skipped_nobox[0] += 1
                        continue
                    try:
                    ...
                    rec = {
                        ...
                        "src_shard": os.path.basename(f), "task": task,
                    ...
                    if task == "detect":
                        ...
                        rec["gt"]["text"] = ((r.get("scene_description") if name.startswith("hayden")
                                              else r.get("image_caption")) or "").strip()
                    elif task == "vqa":
                    ...
                    got += 1
                    if got % 1000 == 0:
                        print("  [%s] %d rows, rotated=%d" % (name, got, sum(1 for m in manifest[-1000:] if m["rotated"])), flush=True)
        stats[name] = {"sampled": got, "scanned": tried, "task": cfg["task"],
    ...
    tag = os.environ.get("COT_TAG", "")
    with open(os.path.join(OUT_ROOT, "manifest%s.jsonl" % tag), "w", encoding="utf-8") as fo:
        for m in manifest:
            fo.write(json.dumps(m, ensure_ascii=False) + "\n")
    with open(os.path.join(OUT_ROOT, "build_stats%s.json" % tag), "w", encoding="utf-8") as fo:
        json.dump({"datasets": stats, "skipped_no_boxes": skipped_nobox[0]}, fo, ensure_ascii=False, indent=1)
    print("manifest rows:", len(manifest), "| 跳过无框行:", skipped_nobox[0])
