#!/bin/bash
# Arm (4): arm② per-step [cls] + arm① additive step_embed (arm①'s original Q-row scope).
# Config identical to arms 1/2/4-controller (single-variable).
export PATH=/root/miniconda3/bin:$PATH
export HF_HUB_OFFLINE=1
export PYTHONPATH=/root/autodl-tmp/sr-diffusion-v3-stepidcls-stepid
cd /root/autodl-tmp/sr-diffusion-v3-stepidcls-stepid || exit 99
LOG=/root/train_logs/stepidcls_stepid_d4_slice05.log
...
accelerate launch --multi_gpu --num_processes 2 --num_machines 1 --same_network train_v2.py \
  ...
  --output_dir output/phase1_v2_stepidcls_stepid_d4_slice05 --model_input 448x252 --canvas 1600x900 --angle_step 0.5 \
