...
class OutputQueryDecoder(nn.Module):
    ...
    def forward(self, z_cls: Tensor, z_s: Tensor) -> Tensor:
        ...
        Y = Y.reshape(B, len(self.steps), Q, D)                  # (B,|T|,N+1,D)
        # 注: [cls] 行输入在 5 步间共享, 输出因读窗(块掩码)不同而不必相同
        self.last_step_cls = Y[:, :, N]                          # 该步 [cls] 槽输出(诊断用)
        Y = Y[:, :, :N]                                          # 仅 patch 行交给像素头/累加损失
