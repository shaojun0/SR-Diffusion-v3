#!/bin/bash
# Arm (3): arm-1 additive step_embed + ONE [cls] slot shared by all 5 steps.
# Config identical to arms 1/2 and to the stack2x controller (single-variable).
export PATH=/root/miniconda3/bin:$PATH
export HF_HUB_OFFLINE=1
export PYTHONPATH=/root/autodl-tmp/sr-diffusion-v3-stepid-shcls
cd /root/autodl-tmp/sr-diffusion-v3-stepid-shcls || exit 99
LOG=/root/train_logs/stepidshcls_d4_slice05.log
...
accelerate launch --multi_gpu --num_processes 2 --num_machines 1 --same_network train_v2.py \
  ...
  --output_dir output/phase1_v2_stepidshcls_d4_slice05 --model_input 448x252 --canvas 1600x900 --angle_step 0.5 \
