...
cd /root/autodl-tmp/sr-diffusion-v3-stepid-cls || exit 99
TLOG=/root/train_logs/stepidcls_d4_slice05.log
POSTF=/root/train_logs/stepidcls_d4_POSTDONE
rm -f "$POSTF"
...
OUT=output/phase1_v2_stepidcls_d4_slice05
...
CUDA_VISIBLE_DEVICES=0 python infer_v2_test.py $D --final_model $OUT/final_model.pt \
  --output $OUT/infer_test.json --batch_size 32 --num_workers 8 > /root/train_logs/stepidcls_d4_infer.log 2>&1
echo "INFER_NEW_EXIT=$?" >> "$POSTF"
CUDA_VISIBLE_DEVICES=0 python probe_step_collapse.py $D --ckpt $OUT/final_model.pt \
  --tag stepidcls_d4_slice05 --limit 512 --bs 16 --num_workers 8 \
  --out output/probe/probe_stepidcls_d4_slice05.json > /root/train_logs/stepidcls_d4_probe.log 2>&1
echo "PROBE_NEW_EXIT=$?" >> "$POSTF"
MI=$OUT/model_info.json
for ck in $OUT/checkpoint-*; do
  [ -d "$ck" ] || continue
  step=${ck##*-}
  o=output/probe/probe_stepidcls_d4_slice05_step${step}.json
  [ -f "$o" ] && continue
...
PY
  ...
  CUDA_VISIBLE_DEVICES=0 python probe_step_collapse.py $D --ckpt "$WP" \
    --tag "stepidcls_d4_slice05_step${step}" --limit 128 --bs 16 --num_workers 8 \
    --out "$o" >> /root/train_logs/stepidcls_d4_ckpt.log 2>&1
  echo "CKPT $step EXIT=$?" >> "$POSTF"
done
python step_gain_analysis.py --md STEP_GAIN_stepidcls.md --json STEP_GAIN_stepidcls.json \
  ...
  --probe stepidcls_d4=output/probe/probe_stepidcls_d4_slice05.json \
  ...
  --infer stepidcls_d4=$OUT/infer_test.json > /root/train_logs/stepidcls_d4_gain.log 2>&1
