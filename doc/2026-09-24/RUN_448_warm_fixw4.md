...
## 0. 结果（2026-09-25 05:26 训练结束；负结果）

**「固定4」计划在主线 448×252 上切换即崩溃、B 相完全没学。**

| 阶段 | 现象 |
|---|---|
| A 相（0–16 ep，单步全读） | 健康：eval_loss 0.477→**0.367**，train loss 0.38，grad_norm 2–14 |
| A→B 切换（step 3504） | **loss 尖峰 11.36，grad_norm 132**，随后在 190~325 与 0.006 之间震荡 |
| B 相（16–56 ep，144 步固定窗口） | **梯度消失**（grad_norm ~3e-3），eval_loss 1.1408→1.1380（后 4 次完全相同），8760 步无有效更新 |
| 全量 test 推理 | L1 **65.59 恒定**（144 个 step 输出逐 step 相同）vs 历史 square24 BPTT **16.42**（19.10→16.42） |

⇒ 模型退化成「与读入/步数无关的常量预测」。机制与 `doc/2026-09-23/REPORT_fixw4_plan.md` §3 一致，但在主线更严重（不是略差，是切换即崩溃）。
完整分析见 `doc/2026-09-24/REPORT_448_warm_fixw4.md`；已 push（commit `d1bdb96`）。
