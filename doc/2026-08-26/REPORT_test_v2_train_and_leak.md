...
### 5.7 像素级逆向还原实验（2026-08-27, 还原失败实证）

用户质疑:「原图丰富多彩, 但还原图都是单色, 按道理不是应该重新堆叠逆向还原吗?」
—— 完全正确。此前结论「目标无空间结构」是**特征空间误判**: 真实 DINO 特征
空间变异性虽小(~5e-5), 但 **1024 维中携带可解码的像素结构**。

**像素可还原性**（32 张 test, 线性最小二乘解码 特征→像素, L1 0-255 尺度）:

| 阶段 | 空间变异性 (within-std) | 像素解码 L1 | 相对全图平均色 |
|---|---|---|---|
| 真实 DINO patch P | 4.79e-5 (基准) | **9.81** | **改善 51.5** |
| ReEncoder 输出 z_s | 5.63e-3 (117%) | 40.39 | 改善 20.9 |
| Decoder 输出 F_hat | 2.89e-6 (**6%**) | **64.87** | **比平均色还差 3.6** |
| 全图平均色参照 | — | 61.32 | 0 |

**像素统计佐证**（可视化图 unique colors）: 原图 143025 → 真实特征解码 42736 →
模型 F_hat 解码 **256** → 全图平均色 220。

**结论（修正 §6）**:
1. **真实 DINO 特征富含像素信息**（解码 L1=9.8, 结构丰富）——目标可还原。
2. **ReEncoder 损失部分像素信息**（9.8 → 40.4）: z_s 变异性虽放大, 但沿不可解码方向。
3. **OutputQueryDecoder 是空间信息丢失的主犯**（40.4 → 64.9）: F_hat 变异性仅剩 6%,
   像素还原退化到「全图平均色」水平 —— 输出退化为每图质心常数。
4. 特征空间 L1=3.8e-5「好」是假象: 目标在特征空间近常数, 模型学到输出质心即达低 L1,
   **但这不是还原, 是退化**。

**下一步方向**（供用户决策）:
- 修 decoder: OutputQueryDecoder 的 KV 因果 + 采样步平均可能压制空间细节;
  可试验 kv_causal=False / 更多采样步 / 输出查询与 patch 位置的显式绑定。
- 或改用像素级目标（F_hat 直接监督像素 patch）, 绕开 DINO 特征空间的退化。
- 指标: 像素解码 L1 应作为还原能力的最终判据, 特征空间 L1 仅为辅助。
## 6. 结论

1. **无 token 泄露**: 掩码数值验证（扰动被屏蔽 token → 输出不变，边界精确）通过。
2. **「1 向量还原」是还原失败的信号**: 特征空间变异性小是表象, 像素级实验证实真实 DINO 特征可解码出结构(L1=9.8)而模型 F_hat 退化为常数(L1=64.9≈平均色)—— OutputQueryDecoder 丢失了空间信息(见 §5.7)。
...
| `edb21fd` | 像素级逆向还原实验: 真实特征可解码(L1=9.8) vs F_hat 退化(L1=64.9), 定位空间信息在 Decoder 丢失 |
