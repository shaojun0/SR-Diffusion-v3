"""
SR-Diffusion Phase 1 v5 — 训练（扩散式渐进细化, K=128, 2026-08-31）
=================================================
架构（model_v5.py）: K=128 special token 进 DINO 序列（1+K+N token, 24 层
    全双向直接算 z_cls, z_s）→ **扩散解码器**（patch 级 DDPM: x_t 加噪,
    x̂0 预测; 条件 = 随 t 递减渐进解锁的 token, NVAE 式 coarse-to-fine）
    → L = MSE(x̂0, x0) + l1_weight·L1(x̂0, x0)。DINO 默认不冻结。
    eval: 固定 t=T//2（确定性 eval_loss）; 推理走 DDIM（infer_v5.py）。

与 v4 的差异: 解码器从"无时序交叉注意力"换成"扩散式渐进细化"——
    步间耦合（x_{t-1} 输入 = x_t）+ 每步目标不同（噪声水平）+ token 渐进
    解锁（m(t): t 大→少 token 粗结构, t 小→全解锁补细节）⇒ 打破 v2 的
    "悬崖+平台"（token 被强制装增量信息, 渐进曲线应为阶梯）。

学习率: 分组 —— DINO 1.5e-4（微调不宜过高, v2 教训）+ 新模块 3e-4;
    全 fp32; 显式优化器只收可训练参数。
    ddp_find_unused_parameters=True: CFG 无条件路径随机跳过交叉注意力,
    DDP 需要允许"偶发未用参数"。

用法:
    accelerate launch --multi_gpu --num_processes 2 \
        train_v5.py --data_dir /root/autodl-tmp/construction_site \
...
def parse_args():
    ...
    p.add_argument("--num_specials", type=int, default=128,
                   help="K: register special token 数(固定压缩维度, 最大 128)")
    p.add_argument("--freeze_dino", action="store_true",
                   help="冻结 DINO(默认不冻结; 冻结=对照)")
    # ── 模型（扩散解码器）──
    p.add_argument("--diffusion_steps", type=int, default=1000,
                   help="DDPM 噪声步数 T")
    p.add_argument("--decoder_depth", type=int, default=4,
                   help="扩散解码器 DiT block 数")
    p.add_argument("--heads", type=int, default=8)
    p.add_argument("--mlp_ratio", type=float, default=2.0)
    p.add_argument("--l1_weight", type=float, default=0.5,
                   help="L1 辅助损失权重(L2 主损失 + l1_weight·L1)")
    p.add_argument("--cfg_drop", type=float, default=0.1,
                   help="训练时丢弃条件(token)的概率(CFG 无条件路径; 0=关闭)")
    p.add_argument("--unlock", default="linear", choices=["linear", "sqrt", "none"],
                   help="渐进解锁计划: linear/sqrt(更早解锁更多)/none(不渐进, 对照)")
    p.add_argument("--eval_t", type=int, default=-1,
                   help="eval 固定噪声步(默认 -1 = T//2)")
    return p.parse_args()
...
def compute_metrics(eval_pred):
    ...
    if isinstance(preds, dict):
        ...
        # 无 labels 路径: 除 loss 外按 dict 插入序 (recon,x0_hat,target,z_s,t,m)
        items = dict(zip(("recon", "x0_hat", "target", "z_s", "t", "m"), preds))
    else:
        items = {}
    metrics = {}
    for k in ("loss", "recon"):
...
class SRPhase1V5Trainer(Trainer):
...
def main():
    ...
    eval_t = args.eval_t if args.eval_t > 0 else args.diffusion_steps // 2
    ...
    # ── 模型: register K 压缩 + 扩散解码器（DINO 默认不冻结）──
    ...
    model = SRPhase1V5(
        ...
        l1_weight=args.l1_weight, cfg_drop=args.cfg_drop,
        unlock=args.unlock, fixed_eval_t=eval_t,
    )

    n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
    n_all = sum(p.numel() for p in model.parameters())
    n_dec = sum(p.numel() for p in model.decoder.parameters())
    print(f"[model] 可训练 {n_train / 1e6:.1f}M / 总 {n_all / 1e6:.1f}M "
          f"(DINO {'冻结' if args.freeze_dino else '不冻结'}); 扩散解码器 "
          f"{n_dec / 1e6:.1f}M; K={args.num_specials} specials "
          f"(序列 {1 + args.num_specials + num_patches} token); "
          f"T={args.diffusion_steps}, unlock={args.unlock}, "
          f"eval_t={eval_t} (m={model.unlock_count(eval_t)})")
    ...
    training_args = TrainingArguments(
        ...
        eval_accumulation_steps=2,          # eval 输出(B,N,588)分批移 CPU
    ...
    if trainer.accelerator.is_main_process:
        ...
        info = {"input_size": [W, H], "canvas": [CW, CH],
                ...
                "mlp_ratio": args.mlp_ratio,
                "l1_weight": args.l1_weight, "cfg_drop": args.cfg_drop,
                "unlock": args.unlock, "eval_t": eval_t,
                "freeze_dino": args.freeze_dino,
                "lr": args.lr, "dino_lr": args.dino_lr,
                "target": "pixel_values (归一化空间, 扩散 x0 预测)",
