"""
SR-Diffusion v4: SVD U+V + DINOv2-giant → SD 2.1 U-Net Diffusion (512×512)
PreTrainedModel 兼容 — 支持 save_pretrained / AutoModel.from_pretrained.

Pipeline:
  1. HR → SVD → top-k left + right singular vectors (U + Vh)
  2. eig_U + eig_V + LR tokens → DINOv2 Encoder → cross-attn tokens (1536d)
  3. LR → VAE Encoder → condition latent
  4. HR → VAE Encoder → +noise → U-Net(cross_attn) → ε pred
  5. Loss: ε-MSE (standard DDPM)
...
class SRDiffusionConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.auto_map = {
            "AutoConfig": "model_v4.SRDiffusionConfig",
            "AutoModel": "model_v4.SRDiffusion",
        }

# ═══════════════════════════════════════════════════════════════
# SVD Preprocessing: U (row) + Vh (column) dual eigenvectors
# ═══════════════════════════════════════════════════════════════

def svd_eigenvectors_uv(
    img: Tensor,
    energy_threshold: float = 0.95,
    max_n: int = 64,
    min_n: int = 16,
):
    """(B, 1, H, W) grayscale → (U, Vh) dual singular vectors.

    U encodes row-wise structure (horizontal textures, edges).
    Vh encodes column-wise structure (vertical textures — rebar, scaffolding).

    Returns:
        eig_u: (B, max_n, H) — left singular vectors (row features)
        eig_v: (B, max_n, W) — right singular vectors (column features)
        n_list: list[int] — actual n per sample
    """
    if img.dim() == 4:
        img = img[:, 0].float()
    else:
    ...
    B, H, W = img.shape
    u_padded, v_padded, n_list = [], [], []

    for b in range(B):
        u, s, vh = torch.linalg.svd(img[b], full_matrices=False)
        # u: (H, min(H,W)), s: (min(H,W),), vh: (min(H,W), W)
        s2 = s ** 2
        n = torch.searchsorted(s2.cumsum(0), energy_threshold * s2.sum()).item() + 1
        n = max(min_n, min(n, max_n))

        u_padded.append(u[:, :n].T)       # (n, H) — row eigenvectors
        v_padded.append(vh[:n, :])         # (n, W) — column eigenvectors (Vh rows)
        n_list.append(n)

    max_nb = max(n_list)
    u_out = torch.zeros(B, max_nb, H, device=img.device, dtype=img.dtype)
    v_out = torch.zeros(B, max_nb, W, device=img.device, dtype=img.dtype)
    for b, (ut, vt, n) in enumerate(zip(u_padded, v_padded, n_list)):
        u_out[b, :n] = ut
        v_out[b, :n] = vt
    return u_out, v_out, n_list
...
def build_tokens(
    ...
    """HR (B, 3, H, W) → lr_flat, eig_u (B,N,H), eig_v (B,N,W), n_list"""
    B, _, H, W = hr.shape
    ...
    eig_u, eig_v, n_list = svd_eigenvectors_uv(
        hr_gray.unsqueeze(1), energy_threshold, max_n, min_n
    )
    return lr_flat, eig_u, eig_v, n_list
...
# ═══════════════════════════════════════════════════════════════
# DINOv2 Encoder — SVD U+V tokens → cross-attention features
# ═══════════════════════════════════════════════════════════════

class DinoEncoder(nn.Module):
    """DINOv2-giant encoder wrapper. All params trainable.
    
    Accepts dual SVD tokens:
      - eig_u (B, N, H): row structure from left singular vectors
      - eig_v (B, N, W): column structure from right singular vectors
    """
    ...
    def __init__(self, cfg: SRDiffusionConfig):
        ...
        self.eig_proj_u = nn.Linear(cfg.image_size, cfg.dino_hidden)  # U row vectors
        self.eig_proj_v = nn.Linear(cfg.image_size, cfg.dino_hidden)  # Vh column vectors
        # pos: CLS + LR + max_eig U + max_eig V = 2*max_eig + 2 tokens → round to +3 for safety
        self.pos = nn.Parameter(
            torch.randn(1, cfg.max_eig * 2 + 3, cfg.dino_hidden) * 0.02
        )
        self.cls = nn.Parameter(torch.randn(1, 1, cfg.dino_hidden))

    def forward(
        self, lr_flat: Tensor, eig_u: Tensor, eig_v: Tensor, n_list: list  # noqa: ARG002
    ) -> Tensor:
        """U + V dual eigenvectors → DINO cross-attention tokens.
        
        Token order: CLS | LR | U_0 ... U_n | V_0 ... V_n
        """
        B = lr_flat.size(0)

        # ── Project U (row vectors, dim=H) ──
        H_actual = eig_u.size(2)
        H_expected = self.eig_proj_u.in_features
        if H_actual != H_expected:
            B_eig, N_eig = eig_u.shape[0], eig_u.shape[1]
            eig_u = eig_u.reshape(B_eig * N_eig, 1, H_actual)
            eig_u = F.interpolate(eig_u, size=H_expected, mode="linear", align_corners=False)
            eig_u = eig_u.reshape(B_eig, N_eig, H_expected)

        eig_u_tok = self.eig_proj_u(eig_u)   # (B, N_u, dino_hidden)

        # ── Project Vh (column vectors, dim=W) ──
        W_actual = eig_v.size(2)
        W_expected = self.eig_proj_v.in_features
        if W_actual != W_expected:
            B_eig, N_eig = eig_v.shape[0], eig_v.shape[1]
            eig_v = eig_v.reshape(B_eig * N_eig, 1, W_actual)
            eig_v = F.interpolate(eig_v, size=W_expected, mode="linear", align_corners=False)
            eig_v = eig_v.reshape(B_eig, N_eig, W_expected)

        eig_v_tok = self.eig_proj_v(eig_v)   # (B, N_v, dino_hidden)

        # ── Assemble: CLS + LR + U + V ──
        lr_tok = self.tok_proj(lr_flat).unsqueeze(1)                     # (B, 1, dino_hidden)
        tokens = torch.cat([lr_tok, eig_u_tok, eig_v_tok], dim=1)        # (B, 1+N_u+N_v, dino_hidden)
        cls_tok = self.cls.expand(B, -1, -1)
        tokens = torch.cat([cls_tok, tokens], dim=1)                     # (B, 2+N_u+N_v, dino_hidden)
...
# ═══════════════════════════════════════════════════════════════
# Diffusion Decoder — SD 2.1 U-Net + VAE (cond_fusion)
# ═══════════════════════════════════════════════════════════════
...
class SRDiffusion(PreTrainedModel):
    ...
    v4: U+V dual SVD eigenvectors for row+column structure encoding.
    ...
    def build_model(
        ...
        print("=" * 60)
        print("SR-Diffusion v4: build_model (U+V dual SVD) — loading pretrained weights")
        print("=" * 60)
        ...
        if dino_dir is not None:
            ...
            print(f"    Loaded {len(our_state)} keys, "
                  f"missing={len(missing)} (new v4 params: eig_proj_v, etc), "
    ...
    def forward(self, hr: Tensor, return_dict: bool = True, **kwargs):
        ...
        # ── 1. SVD U+V tokens via DINOv2 ──
        lr_flat, eig_u, eig_v, n_list = build_tokens(
            hr, cfg.lr_size, cfg.energy_threshold, cfg.max_eig, cfg.min_eig,
        )
        svd = self.dino(lr_flat, eig_u, eig_v, n_list)
    ...
    def sample(self, hr: Tensor, steps: int = 25) -> Tensor:
        ...
        cross = self.projector(self.dino(lr_flat, eig_u, eig_v, n_list))
