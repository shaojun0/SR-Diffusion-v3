#!/usr/bin/env bash
# eval_224_d4_bptt.sh — 等 224x126 / depth=4 / BPTT 训练结束 → 自动评测
#
# 模型: /root/autodl-tmp/construction_site/out_224_d4_bptt/final_model.pt
#  输入 224x126 ⇒ patches 16x9=144, specials=144, 采样 12 步 [1,4,...,144]
#  训练集 construction_site train 7,009 张; 测试集 test 3,004 张
#
# 评测三件事（用户 2026-09-22 口径：'测试的时候也顺便在训练集里抽部分数据'）:
#   1) test  3,004 张（schema v2: PSNR/MS-SSIM/内容区/平凡基线/逐图逐step）
#   2) train 随机 1,000 张（seed=42 ⇒ 看 train/test 差距 = 过拟合程度）
#   3) 经典 codec 基线在同一 224x126 画布上重跑（JPEG/WebP）⇒ 同 bpp 轴可比
set -uo pipefail
cd "$(dirname "$0")/.."
...
OUT=${OUT:-/root/autodl-tmp/construction_site/out_224_d4_bptt}
MODEL=$OUT/final_model.pt
EVAL=${EVAL:-/root/autodl-tmp/cot_l1/eval_224_d4}
...
infer "$CS" "$EVAL/test_224_d4_bptt.json" 0 2>&1 | tee "$EVAL/test.log"

echo "[eval] (2/3) train 随机 1,000 张"
python tools/make_train_sample.py --data_dir "$CS" --out_dir "$TRAIN1K" --n 1000 --seed 42
infer "$TRAIN1K" "$EVAL/train1k_224_d4_bptt.json" 0 2>&1 | tee "$EVAL/train1k.log"
