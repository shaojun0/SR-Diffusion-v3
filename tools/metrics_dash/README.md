...
| 端口 | 服务 | 内容 | 公网地址 |
|---|---|---|---|
| **6006** | TensorBoard | scalars 事件文件（loss / grad_norm / lr / eval_recon / 探针） | `https://u849831-me4w-bf7ae4cd.westb.seetacloud.com:8443` |
| **6008** | Streamlit | 交互看板（10 张 Plotly 图 + 汇总表 + 数据源状态） | `https://uu849831-me4w-bf7ae4cd.westb.seetacloud.com:8443` |
...
| # | 数据 | 路径 | 当前状态 |
|---|---|---|---|
| 1 | 新实验训练日志（**持续增长**） | `/root/train_logs/stack2x_slice05.log` | ✅ 有数据（训练中，约 1.94 s/it，40 epoch / 8760 step） |
| 1 | 基线训练日志 | `/root/train_logs/blockdiag_slice05.log` | ✅ 完整（438 个 train 点 + 5 个 eval 点，final eval_recon **0.3318**） |
| 2 | 新模型探针 | `/root/autodl-tmp/sr-diffusion-v3-stack2x/output/probe/probe_stack2x_slice05.json` | ⏳ **待训练结束后生成** |
| 2 | 基线探针 | `.../output/probe/probe_baseline_blockdiag_slice05.json` | ⏳ 该路径暂缺；已自动回落到 `/root/train_logs/probe_blockdiag.json` ✅ |
| 3 | 新模型全量推理 | `.../output/phase1_v2_stack2x_slice05/infer_test.json` | ⏳ 待生成 |
...
| # | 图 | 数据源 | 现在有数据？ |
|---|---|---|---|
| ① | train loss vs step（新 + 基线同图） | 训练日志 | ✅ 基线 438 点；新实验持续增长中 |
| ② | 梯度平均变化 grad_norm vs step（对数轴可切） | 训练日志 | ✅ |
| ③ | learning_rate vs step | 训练日志 | ✅ |
| ④ | eval_recon vs step + 基线 0.3318 虚线 | 训练日志 eval 行 | ✅ 基线 5 点（0.486→0.4199→0.3525→0.3325→**0.3318**）<br>⏳ 新实验 0 点（eval_every=2000，尚未到） |
| ⑤ | **step1~5 的平均值** `step_px_scale`（柱状，新 vs 基线） | 探针 JSON | ⏳ 新实验待生成；✅ 基线已回落加载 |
| ⑥ | `step_px_scale` 各采样步随训练进程变化 | 多 checkpoint 探针 | ⏳ 目前每个 run 只有 1 个探针文件 |
| ⑦ | `E_px` step × region 热力图（0-255 L1） | 探针 JSON | ⏳ 新实验待生成；✅ 基线可看 |
| ⑧ | `E_nrm` 热力图（折叠面板） | 探针 JSON | 同上 |
| ⑨ | `prog_curve_255` 渐进曲线 | 探针 JSON | 同上 |
| ⑩ | `z_s_within_std` / 块内 cos | 探针 JSON | 同上 |
| ⑪ | 全量推理 `full_pixel_l1_255` / `step_pixel_l1_255` | infer JSON | ⏳ 两个 run 都待生成 |
| ⑫ | 数据源状态表（OK / 缺失清单） | — | ✅ |

TensorBoard 侧当前 tag 数：`baseline_blockdiag_slice05` **47** 个、`stack2x_slice05` **4** 个
（新实验探针未生成，故只有 train/*）。
...
1. **JSON 里带 `step` 字段**（最稳）：
   ```json
   { "step": 4000, "tag": "stack2x_step4000", "step_px_scale": [...], ... }
   ```
   也认 `train_step` / `global_step` / `ckpt_step` / `steps_trained`。
2. **文件名带 step**：`probe_stack2x_slice05_step4000.json`、
   `probe_stack2x_ckpt6000.json`、`probe_stack2x_checkpoint-8000.json`
...
```bash
# 例：训练途中每 2000 步存了 checkpoint，对 checkpoint-4000 跑一次探针
# （用既有链路 probe_step_collapse.py 生成，注意：要用空闲 GPU 或 CPU，别抢训练资源）

# 方式 A：直接放进自动扫描目录，文件名带 step
cp probe_result.json \
   /root/autodl-tmp/sr-diffusion-v3-stack2x/output/probe/probe_stack2x_slice05_step4000.json

# 方式 B：放到 /root/train_logs 并命名成 probe_stack2x_slice05_step4000.json（同样会被扫到）

# 然后等下一个刷新周期（≤45s），或立刻手动刷一次：
cd /root/sr-metrics-dash && bash run_dash.sh refresh
```

> 扫描规则见 `parse_metrics.py` 的 `RUNS[...]["probe_globs"]`：
> 新实验匹配 `probe_*stack2x*slice05*.json`，基线匹配 `probe_*baseline*blockdiag*slice05*.json`
> 与回落的 `probe_*blockdiag*.json`。要加别的命名，改这里即可。
...
**Q: 新实验的 loss 曲线为什么只有几十个点？**
日志是每 20 step 一行，训练 1.94 s/it，约 6.5 分钟才出一行。跑满 8760 步约有 438 行。

**Q: 图④ eval_recon 新实验是空的？**
