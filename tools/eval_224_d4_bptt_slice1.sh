#!/usr/bin/env bash
# eval_224_d4_bptt_slice1.sh — 等「224×126 / d4 / BPTT / slice[1:12]」训练结束 → 自动评测
#
# 模型: /root/autodl-tmp/construction_site/out_224_d4_bptt_slice1/final_model.pt
#  输入 224×126 ⇒ patches 16×9=144, specials(K)=144, 采样 **11 步** [4,9,…,144]
#  slice_start/slice_end/decoder_steps/num_specials **全部由 model_info.json 驱动**
#  推理侧（infer_v2_test.py 的 _pick），命令行不传这些参数。
#
# 评测三件事（与基线 tools/eval_224_d4_bptt.sh 同口径, 便于逐行对比）:
#   1) test  3,004 张（schema v2: PSNR/MS-SSIM/内容区/平凡基线/逐图逐step）
#   2) train 随机 1,000 张（seed=42 ⇒ 看 train/test 差距 = 过拟合程度）  ← 用户口径
...
EVAL=${EVAL:-/root/autodl-tmp/cot_l1/eval_224_d4_slice1}
...
infer "$CS" "$EVAL/test_224_d4_bptt_slice1.json" 0 2>&1 | tee "$EVAL/test.log"

echo "[eval] (2/3) train 随机 1,000 张"
python tools/make_train_sample.py --data_dir "$CS" --out_dir "$TRAIN1K" --n 1000 --seed 42
infer "$TRAIN1K" "$EVAL/train1k_224_d4_bptt_slice1.json" 0 2>&1 | tee "$EVAL/train1k.log"
